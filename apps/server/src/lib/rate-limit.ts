...
export const defaultAnonPerHour = (): number =>
  envNumber('FLOOM_RATE_LIMIT_IP_PER_HOUR', 20);
export const defaultUserPerHour = (): number =>
  envNumber('FLOOM_RATE_LIMIT_USER_PER_HOUR', 200);
export const defaultPerAppPerHour = (): number =>
  envNumber('FLOOM_RATE_LIMIT_APP_PER_HOUR', 50);
...
export function runRateLimitMiddleware(
  ...
  return async (c, next) => {
    ...
    if (!p.allowed) return rateLimitResponse(c, primaryScope, p.retryAfterSec);
    ...
    if (slug) {
      ...
      if (!r.allowed) return rateLimitResponse(c, 'app', r.retryAfterSec);
    }
    return next();
  };
}
