...
export interface Manifest {
   ...
   *
   * KNOWN GAP: the e2b JS SDK `Sandbox.create` does NOT currently expose a
   * per-instance memory knob. `memoryMB` is only settable at Template build
   * time (see `TemplateBuildRequest.memoryMB`). For MVP we surface the field
   * in the manifest and record it in metadata, and use it later when the
   * template is (re)built. If the detected value is >512 we also emit a
   * warning via the logger so the caller knows the default template may OOM.
   * See h2-full-tests.md "Suite D" for the OOM at 512 MB finding.
   */
  memoryMb?: number;
  ...
  /**
   * Egress allowlist (host:port patterns). Not enforced by e2b today (it
   * allows all outbound by default, see Suite D). Recorded for future
   * enforcement.
   */
