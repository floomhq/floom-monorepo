...
class PostgresStorageAdapter {
    ...
    async claimJob(id) {
        return one((await this.query(`UPDATE jobs
            SET status = 'running',
                started_at = now(),
                attempts = attempts + 1
          WHERE id = $1
            AND status = 'queued'
          RETURNING *`, [id])).map(normalizeJob));
    }
    async claimNextJob() {
    ...
    async markJobComplete(id, outputs, run_id) {
        return one((await this.query(`UPDATE jobs
            SET status = 'succeeded',
                ...
                finished_at = now()
          WHERE id = $3
          RETURNING *`, [toNullableJson(outputs), run_id, id])).map(normalizeJob));
    }
    async markJobFailed(id, error, run_id) {
        return one((await this.query(`UPDATE jobs
            SET status = 'failed',
                error_json = $1::jsonb,
                run_id = $2,
                finished_at = now()
          WHERE id = $3
          RETURNING *`, [toNullableJson(error), run_id, id])).map(normalizeJob));
    }
    async cancelJob(id) {
        const cancelled = one((await this.query(`UPDATE jobs
    ...
    async listAdminSecrets(app_id, ctx) {
        if (app_id === undefined) {
            if (ctx?.workspace_id) {
                return (await this.query(`SELECT secrets.*
             FROM secrets
             INNER JOIN apps ON apps.id = secrets.app_id
            WHERE apps.workspace_id = $1
            ORDER BY secrets.name`, [ctx.workspace_id])).map(normalizeSecret);
            }
        ...
        if (ctx?.workspace_id) {
          ...
          WHERE secrets.app_id = $1
            AND apps.workspace_id = $2
          ORDER BY secrets.name`, [app_id, ctx.workspace_id])).map(normalizeSecret);
        }
