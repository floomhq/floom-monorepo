#!/usr/bin/env bash
# redeploy-preview.sh — canonical redeploy for floom-preview-launch (preview.floom.dev).
#
...
# every restart. The previous floom-deploy-preview.sh in /usr/local/sbin/
...
#   2. Capture the running container's secrets, layer preview.env.canonical
#      on top.
...
#
# Usage:
#   bash scripts/redeploy-preview.sh                # builds from HEAD
#   bash scripts/redeploy-preview.sh promote        # custom tag suffix
#   IMAGE_TAG=floom-preview:foo bash scripts/redeploy-preview.sh
#
...
CANONICAL_ENV="${REPO_ROOT}/scripts/preview.env.canonical"
KILL_LIST="${REPO_ROOT}/scripts/launch-kill-list.txt"
CONTAINER=floom-preview-launch
HOST_PORT=3052
CONTAINER_PORT=3000
HEALTH_URL="http://127.0.0.1:${HOST_PORT}/api/health"
IMAGE_REPO=floom-preview

BINDS=(
  "-v" "floom-preview-launch-data:/data"
  "-v" "/opt/floom-preview-apps:/apps"
  "-v" "/opt/floom-launch-file-inputs:/floom-file-inputs"
)

log() { printf '\033[36m[redeploy-preview]\033[0m %s\n' "$*"; }
err() { printf '\033[31m[redeploy-preview]\033[0m %s\n' "$*" >&2; }
...
# Prod-specific guard: PUBLIC_URL MUST be https://preview.floom.dev so generated URLs
# don't leak the preview origin.
if env.get("PUBLIC_URL") != "https://preview.floom.dev":
    print(f"FATAL: prod PUBLIC_URL != https://preview.floom.dev (got {env.get('PUBLIC_URL')!r})", file=sys.stderr)
    sys.exit(4)
...
if [ "$LIVE_PUBLIC_URL" != "https://preview.floom.dev" ]; then
  err "FATAL: PUBLIC_URL != https://preview.floom.dev in running container (got '$LIVE_PUBLIC_URL')"
  exit 1
fi

log "OK: container healthy, FLOOM_CLOUD_MODE=1 + PUBLIC_URL=https://preview.floom.dev verified"
log "image: $TAG"
log "url:   https://preview.floom.dev"
