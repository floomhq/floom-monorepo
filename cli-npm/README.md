...
```bash
npx @floomhq/cli@latest login
```

This opens https://floom.dev/home in your browser, asks you to paste an agent token, validates it, saves it to `~/.floom/config.json`, and prints the MCP config snippet.

## Other commands

```bash
floom auth <agent-token>     # save token non-interactively
floom login                  # open browser token flow
