...
function validateToken(apiUrl, token) {
  const url = new URL('/api/session/me', apiUrl.replace(/\/$/, '') + '/');
...
async function runSetup(opts) {
  ...
  // Save config.
  const valid = await validateToken(apiUrl, token);
  if (!valid) {
    console.log('');
    console.error(c.red(`  Token rejected by ${apiUrl}.`));
    console.error(`  Mint a fresh token at ${tokenUrl} and try again.`);
    process.exit(1);
  }
...
function forwardToBash(args, opts = {}) {
  ...
  if (opts.apiUrl) env.FLOOM_API_URL = opts.apiUrl;
  const result = spawn('bash', [BUNDLED_BASH, ...args], { stdio: 'inherit', env });
...
${c.bold('usage:')}
  ...
  floom login                  ${c.dim('# alias for setup')}
...
async function main() {
  ...
  // Parse --api-url if present. The bundled bash CLI reads FLOOM_API_URL.
